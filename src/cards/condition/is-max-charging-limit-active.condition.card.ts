...
export class IsMaxChargingLimitActiveConditionCard implements RunListener {
    run(args: { device: HomePowerStation; [key: string]: unknown }, state: Record<string, unknown>): Promise<unknown> {
        return new Promise<unknown>(async (resolve, reject) => {
            ...
            hps.log('Starting card to check if the max charging limit is active')
            hps.getApi().readChargingConfiguration(true, hps.asSimple())
                .then(config => {
                    resolve(
                        config.currentLimitations.chargingLimitationsEnabled
                            && config.currentLimitations.maxCurrentChargingPower < config.maxPossibleChargingPower)
                })
                .catch(e => {
